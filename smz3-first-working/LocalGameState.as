...
class LocalGameState : GameState {
  ...
  bool is_frozen() {
    return (bus::read_u8(0x7E02E4) != 0) && rom.is_alttp();
  }
  ...
  void fetch_sram() {
	...
	if(rom.is_alttp()){
		...
		bus::read_block_u8(0x7E0900, 0, 0x500, sram);
	}
  }
  ...
  void send() {
    ...
    if (!settings.RaceMode) {
      ...
      if ((frame & 1) == 0) {
        array<uint8> envelope1 = create_envelope(0x02);
        serialize_torches(envelope1);
  ...
  void update_items() {
	...
	if(rom.is_alttp()){
		finish_update(rom.syncables);
	}
	else {
		finish_update(rom.syncables_metroid);
	}
  }
  
  void finish_update(array<SyncableItem@> syncables){
  ...
  void finish_update(array<SyncableSMZ3@> syncables){
	// track names of items received:
    received_items.reserve(syncables.length());
    received_items.resize(0);
    received_quests.reserve(16);
