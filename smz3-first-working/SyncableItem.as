...
class SyncableSMZ3 : SyncableItem{
	
	SyncableSMZ3(uint16 offs, uint8 size, uint8 type, NotifyNewItems @notifyNewItems = null){
		super(offs, size, type, @notifyNewItems);
	}
	
	SyncableSMZ3(uint16 offs, uint8 size, ItemMutate @mutate, NotifyNewItems @notifyNewItems = null){
		super(offs, size, mutate, @notifyNewItems);
	}
	
	void write(uint16 newValue) override{
    if (size == 1) {
      bus::write_u8(0x7E0900 + offs, uint8(newValue));
    } else if (size == 2) {
      bus::write_u16(0x7E0900 + offs, newValue);
    }
  }
